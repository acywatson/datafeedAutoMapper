...
<xsl:template match = "/rss/channel">
	...
	<xsl:for-each select = "item">
		...
		<!-- Sale Price Effective Date --><xsl:value-of select = "normalize-space(g:sale_price_effective_date)" /><xsl:text>&#009;</xsl:text>
		<!-- strProductSKU3 --><xsl:value-of select = "normalize-space(g:sku)" /><xsl:text>&#009;</xsl:text>
		<!-- Short Description --><xsl:value-of select = "normalize-space(g:short_description)" /><xsl:text>&#009;</xsl:text>
		<!-- Weight --><xsl:value-of select = "normalize-space(g:weight)" /><xsl:text>&#009;</xsl:text>
		<!-- Visibility --><xsl:value-of select = "normalize-space(g:visibility)" /><xsl:text>&#009;</xsl:text>
		<!-- Status --><xsl:value-of select = "normalize-space(g:status)" /><xsl:text>&#009;</xsl:text>
		<!-- Buy URL 3 --><xsl:value-of select = "normalize-space(g:url)" /><xsl:text>&#009;</xsl:text>
		<!-- Product SKU3 --><xsl:value-of select = "normalize-space(g:product_id)" /><xsl:text>&#009;</xsl:text>
		<!-- Large Image 2--><xsl:value-of select = "normalize-space(g:base_image)" /><xsl:text>&#009;</xsl:text>
		<!-- Quantity --><xsl:value-of select = "normalize-space(g:qty)" /><xsl:text>&#009;</xsl:text>
		<!-- Parent Id --><xsl:value-of select = "normalize-space(g:parent_id)" />
		<xsl:text>
</xsl:text>
