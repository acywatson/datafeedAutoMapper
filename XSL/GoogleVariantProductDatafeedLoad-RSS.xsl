...
<xsl:template match = "/rss/channel">
	...
	<xsl:for-each select = "item">
		...
		<xsl:choose>
			<xsl:when test = "g:ID">
				<xsl:value-of select = "normalize-space(g:ID)" />
			</xsl:when>
			<xsl:otherwise>
					<xsl:value-of select = "normalize-space(g:id)" />
			</xsl:otherwise>
		...
		<!-- Manufacturer Part Number 2 --><xsl:value-of select = "normalize-space(g:model_number)" />
	</xsl:if>
